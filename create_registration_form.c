...
void create_training_registration_form(Employee_training_data **first_field)
{
	...
	closedir(dr);
        //To take user input for training names
	while(1)
	{
		int flag2 = 0, flag6 = 0;
		...
		if(flag2 == 1)
		{
			...
			if(invalid_form != 3)
			{
				printf("\nInfo: Please enter valid training name\n\n");
				continue;
			}
			else
			{
				printf("\nError: User entered invalid training name three times, returning to previous name\n\n");
				registration();
			}
		}
		else
	...
	for(int i = 0;i < Forms_count; i++)
	{
		if(strcasecmp(Form[i], form) == 0)
		{
			flag3 = 1;
		}
	}
	strcpy(Form[Forms_count], form); //Copy the content of form to Forms[Forms_count])
	int invalid_count = 0;
	if(flag3 == 0)
	{ 
		while(1)
		{
				...
				for(int i = 0;i < field_count;i++)
				{
					...
					while(1)
					{
						...
						if(flag4 == 0)
						{
							...
							{
								printf("\nInfo: Entered field is already present\n\n");
								continue;
							}
							break;
						}
						else
						{
							...
							if(invalid_field != 3)
							{
								printf("\nInfo: Please enter valid field name\n\n");
								continue;
							}
							else
							{
								printf("\nError: User entered invalid field name three times, returning to previous menu\n\n");
								remove(form);
								registration();
							}
						}
			...
			else
			{
				...
				if(invalid_count != 3)
				{
					printf("\nInfo: Invalid field count\n\n");
					continue;
				}
				else
				{
					printf("\nError: User entered invalid field count for three times, returning to previous menu\n\n");
					remove(form);
					registration();
				}
			}
